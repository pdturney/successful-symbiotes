...
# (a family of fusions). From each family, select a fusion seed
# that has no children (a leaf node in the family tree).
#
import golly as g
...
#
fusion_table_file = family_tree_dir + "/fusion_tables_leaf.tsv"
fusion_table_handle = open(fusion_table_file, "w")
#
# Make 12 tables for leaf nodes:
#
...
for i in range(num_runs):
  ...
  for fusion_seed_ID in fusion_seed_identifiers:
    ...
    #
    # - leaf_node_ID (ID of seed with least children)
    # - leaf_node_count (count of seed with least children)
    #
    # initialize leaf_node: leaf_node will eventually be updated
    # to have minimum number of children
    leaf_node_ID = fusion_seed_ID
    leaf_node_count = num_children
    ...
    for j in range(num_descendants):
      ...
      # update leaf_node?
      if (children_count < leaf_node_count):
        leaf_node_ID = parent_ID
        leaf_node_count = children_count
    #
    # - number of nodes in tree (number of descendants)
    #
    num_nodes = len(descendants_properties)
    #
    # - leaf mutualism = [leaf_insider_count, leaf_outsider_count,
    #                     leaf_insider_growth, leaf_outsider_growth]
    #
    #                [red, blue,   orange,     green]
    colour_weights = [1.0, 0.0, (2.0 / 3.0), (1.0 / 3.0)]
    num_steps = 1000
    leaf_node = map_ID_to_seed[leaf_node_ID]
    [leaf_insider_count, leaf_outsider_count, \
      leaf_insider_growth, leaf_outsider_growth] = \
      mfunc.seed_mutualism_stats(g, leaf_node, num_steps, colour_weights)
    #
    # - leaf interaction = [leaf_ensemble_count, leaf_soloist_count,
    #                       leaf_ensemble_growth, leaf_soloist_growth]
    #
    [leaf_ensemble_count, leaf_soloist_count, \
      leaf_ensemble_growth, leaf_soloist_growth] = \
      mfunc.seed_interaction_stats(g, leaf_node, num_steps)
    #
    # - leaf management = [leaf_manager_count, leaf_worker_count,
    #                      leaf_manager_growth, leaf_worker_growth]
    #
    [leaf_manager_count, leaf_worker_count, \
      leaf_manager_growth, leaf_worker_growth] = \
      mfunc.seed_management_stats(g, leaf_node, num_steps)
    #
    # update the tables
    #
    man_cnt[leaf_manager_count, leaf_worker_count] += 1
    man_sum[leaf_manager_count, leaf_worker_count] += \
      leaf_manager_growth + leaf_worker_growth
    #
    mut_cnt[leaf_outsider_count, leaf_insider_count] += 1
    mut_sum[leaf_outsider_count, leaf_insider_count] += \
      leaf_outsider_growth + leaf_insider_growth
    #
    int_cnt[leaf_soloist_count, leaf_ensemble_count] += 1
    int_sum[leaf_soloist_count, leaf_ensemble_count] += \
      leaf_soloist_growth + leaf_ensemble_growth
    #
    #
  #
