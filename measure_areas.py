...
# Measure Areas
#
# Peter Turney, January 18, 2021
#
# Calculate the average area of each seed in the elite
...
analysis_path = analysis_dir + "/measure-areas-" + \
  basename + ".tsv"
...
#
# <generation number> <tab> <average seed area for A>
#                     <tab> <average seed area for B>
#                     <tab> <average seed area for C>
#                     ...
#
# -----------------------------------------------------------------
...
for i in range(0, final_num + 1, step_size):
  ...
  for run in range(num_runs):
    ...
    x_handle.close()
    # calculate the average area of the seeds
    total_area = 0.0
    for seed in x_sample:
      area = seed.xspan * seed.yspan
      total_area = total_area + area
    avg_area = total_area / len(x_sample)
    avg_areas.append("{:.1f}".format(avg_area)) # convert to formatted string
  # write out the areas
  tab = "\t"
  mfunc.show_message(g, analysis_handle, \
    str(i) + tab + tab.join(avg_areas) + "\n")
#
# Final message.
#
mfunc.show_message(g, analysis_handle, "\nAnalysis complete.\n")
